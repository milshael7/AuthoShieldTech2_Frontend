...
/* ============================================================
   GLOBAL ADMIN EARTH — CLEAN ADMIN LAYER
   - Compact counter (1, 2, 3K, 2M)
   - Email (draft) + Notification (unread)
   - Direct jump navigation
   - Blended styling (not loud)
============================================================ */

function __formatCompactCount(n) {
  if (!n || n <= 0) return "";
...
function GlobalAdminEarth({
  companyComms,
  getCompanyName,
  setMode,
  setWorkspace,
  setOperatorPanel,
  setSelectedCompanyForComms,
  setSelectedEmailId,
}) {
  ...
  const feed = useMemo(() => {
    ...
    Object.entries(companyComms || {}).forEach(([companyId, bucket]) => {
      const companyName = getCompanyName(companyId);

      (bucket.notifications || []).forEach((n) => {
        if (!n.read) {
          items.push({
            id: n.id,
            type: "notification",
            ...
            title: n.title,
            message: n.message,
            linkedEmailId: n.linkedEmailId || null,
          });
        }
      });

      (bucket.emails || []).forEach((e) => {
        if (e.status === "DRAFT") {
          items.push({
            id: e.id,
            type: "email",
    ...
    return items
      ...
      .slice(0, 200);
  }, [companyComms, getCompanyName]);

  const count = feed.length;
  const compact = __formatCompactCount(count);

  const handleNavigate = (item) => {
    setOpen(false);
    setMode("operator");
    setWorkspace(item.companyId);

    if (item.type === "email") {
      setSelectedCompanyForComms(item.companyId);
      setSelectedEmailId(item.linkedEmailId);
      setOperatorPanel("email");
    } else {
      setSelectedCompanyForComms(item.companyId);
      setOperatorPanel("notifications");
