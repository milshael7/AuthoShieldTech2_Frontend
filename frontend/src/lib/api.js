...
export const api = {
  ...
  // requires header: x-reset-key (your PAPER_RESET_KEY)
  paperReset: (resetKey) =>
    req('/api/paper/reset', {
      method: 'POST',
      body: {},
      headers: { 'x-reset-key': String(resetKey || '') },
    }),

  // (only if you added /api/paper/config on backend)
  // baselinePct: 0.03, maxPct: 0.20, maxTradesPerDay: 40
  paperSetConfig: (payload, adminKey) =>
    req('/api/paper/config', {
      method: 'POST',
      body: payload,
      headers: adminKey ? { 'x-admin-key': String(adminKey) } : {},
    }),
