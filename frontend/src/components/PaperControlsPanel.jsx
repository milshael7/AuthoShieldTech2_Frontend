...
export default function PaperControlsPanel() {
  const [cfg, setCfg] = useState({ baselinePct: 1, maxPct: 5, maxTradesPerDay: 6 });
  const [status, setStatus] = useState(null);

  const [ownerKey, setOwnerKey] = useState(''); // you paste your OWNER key here
  ...
  async function save() {
    ...
    const payload = {
      ...
      maxTradesPerDay: Math.floor(clamp(cfg.maxTradesPerDay, 0, 200)),
    };
  ...
  return (
    <div className="card">
      ...
      <small>Controls how aggressive the paper trader can be. Owner key required to change settings.</small>
        ...
        <>
          ...
          <div className="tableWrap">
            <table className="table">
              <tbody>
                <tr><td><small>Balance</small></td><td><small>{status?.balance ?? '—'}</small></td></tr>
                <tr><td><small>PNL</small></td><td><small>{status?.pnl ?? '—'}</small></td></tr>
                <tr><td><small>Wins</small></td><td><small>{status?.wins ?? '—'}</small></td></tr>
                <tr><td><small>Losses</small></td><td><small>{status?.losses ?? '—'}</small></td></tr>
                <tr><td><small>Trades Today</small></td><td><small>{status?.tradesToday ?? '—'}</small></td></tr>
              </tbody>
            </table>
          </div>

          <div style={{ height: 10 }} />
          <button onClick={load} style={{ width: 'auto', minWidth: 140 }}>
            Refresh status
          </button>
        </>
      )}
