  ...
  return (
    <div className="card">
      ...
      {list.map((n) => {
        ...
        return (
          <div key={n?.id || `${title}-${when}`} className="pill" style={{ marginBottom: 10 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'center' }}>
              ...
              <div className="actions" style={{ maxWidth: 360 }}>
                <ReadAloud text={`${title}. ${message}`} />
                <ShareSnippet text={`${title}\n\n${message}`} />

                {/* Mark read only when allowed + not already read */}
                {!read && canRead && (
            ...
            {message && (
              <div style={{ marginTop: 8, color: 'var(--muted)', wordBreak: 'break-word' }}>
                {message}
              </div>
            )}
          </div>
